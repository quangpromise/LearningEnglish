-- Bo sung kind/file_name cua tin nhan GAN NHAT vao my_conversations() - man
-- Tin nhan can biet tin cuoi la anh/gif/file de hien "[Hinh anh]"/"da gui 1
-- tep" thay vi in thang URL (xem migration 0016_chat_media.sql cho cot
-- kind/file_name tren bang messages).

-- CREATE OR REPLACE khong cho phep doi danh sach cot tra ve (RETURNS TABLE)
-- - phai DROP truoc vi ham nay them 2 cot moi so voi ban goc.
...
as $$
  ...
  left join lateral (
    select m.content, m.created_at, m.sender_id, m.kind, m.file_name
